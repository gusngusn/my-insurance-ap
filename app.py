...
if st.session_state.menu == "📊 메인 대시보드":
    ...
    with col_left:
        ...
        if upcoming_bdays:
            st.dataframe(pd.DataFrame(upcoming_bdays).sort_values(by="D-Day"), use_container_width=True, hide_index=True)
        else:
        ...
        if upcoming_auto:
            st.dataframe(pd.DataFrame(upcoming_auto).sort_values(by="D-Day"), use_container_width=True, hide_index=True)
        else:
        ...
        if not target_res.empty:
            ...
            if not st.session_state[f"edit_{search_name}"]:
                ...
                if not matched.empty:
                    st.table(matched[["가입날짜", "보험회사", "상품명", "금액"]].reset_index(drop=True))
...
# (5) 실적 입력 및 분석
    ...
    with tab_in:
        with st.form("perf_form"):
            ...
            p_date = st.date_input("가입날짜", today)
            p_comp = st.text_input("보험회사"); p_prod = st.text_input("상품명"); p_price = st.text_input("금액")
...
# (6) CSV DB 일괄 업로드
    ...
    if up_file:
        ...
        if st.button("🚀 데이터 병합 전송"):
            ...
            st.success("데이터 병합 완료!"); st.rerun()

# (7) SMS 단체 발송
elif st.session_state.menu == "📩 SMS 단체 발송 (Cloud)":
    st.title("📩 단체 문자 발송 (Cloud)")
    valid_cust = db_cust[db_cust['연락처'].astype(str).str.len() > 8]
    target_group = st.radio("대상 선택", ["직접 선택", "이번달 생일 고객", "전체 고객"], horizontal=True)
    
    selected_recipients = []
    if target_group == "전체 고객": selected_recipients = valid_cust['이름'].tolist()
    elif target_group == "이번달 생일 고객":
        for _, row in valid_cust.iterrows():
            jumin = str(row.get('주민번호', '')).strip()
            if len(jumin) >= 6 and jumin[2:4] == str(today.month).zfill(2): selected_recipients.append(row['이름'])
    elif target_group == "직접 선택": selected_recipients = st.multiselect("수신 고객 선택", valid_cust['이름'].tolist())
            
    template = st.selectbox("템플릿", ["직접 작성", "생일 축하 (LMS)", "안부 인사 (SMS)", "자동차보험 만기 알림 (LMS)"])
    message = st.text_area("내용", height=150)
    
    if st.button("🚀 발송 시작", type="primary"):
        if selected_recipients and message:
            st.success(f"{len(selected_recipients)}명에게 발송되었습니다! (시뮬레이션)")
