...
def get_gsheets():
    try:
        ...
        # 첫 번째 시트 (고객DB)와 두 번째 시트 (보장분석) 호출
...
# --- [2. 데이터 로드 및 초기화] ---
st.set_page_config(page_title="배현우 FC 시스템 v46.0", layout="wide")
sheet1, sheet2 = get_gsheets()

# 첫 번째 시트 헤더: 고객 기본 정보
h1 = ["등록일자", "이름", "주민번호", "연락처", "주소", "직업", "계좌번호", "자동차보험", "자동차보험회사", "가입일자"]
# 두 번째 시트 헤더: 보유계약 리스트
h2 = ["계약자", "가입날짜", "보험회사", "상품명", "금액", "입력시간"]

if sheet1:
    data1 = sheet1.get_all_values()
    db_cust = pd.DataFrame(data1[1:], columns=h1[:len(data1[0])]) if len(data1) > 0 else pd.DataFrame(columns=h1)
else:
