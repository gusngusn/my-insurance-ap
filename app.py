    ...
    if selected_cust != "선택":
        with st.form("contract_form"):
            ...
            if st.form_submit_button("🚀 보장분석 시트로 전송"):
                ...
                else: st.warning("이미 등록된 동일한 계약이 존재합니다.")

# (5) 실적 입력 및 분석 (자동 연동 로직 추가)
    ...
    with tab_in:
        with st.form("perf_form"):
            ...
            p_price = st.text_input("금액 (숫자만)")
            
            if st.form_submit_button("🚀 실적 등록 및 보장분석 자동 반영"):
                if p_name != "고객없음" and p_price:
                    ...
                    # 2. 보장분석 시트 중복 체크 후 자동 저장 (핵심 로직)
                    is_duplicate = not db_contract[(db_contract['계약자'] == p_name) & 
                                                   (db_contract['가입날짜'] == p_date_str) & 
                                                   (db_contract['보험회사'] == p_comp) & 
                                                   (db_contract['상품명'] == p_prod)].empty
                    
                    if not is_duplicate:
                        sheet2.append_row([p_name, p_date_str, p_comp, p_prod, p_price, now_str])
                        st.success(f"✅ {p_name}님 실적 등록 및 보장분석 업데이트 완료!")
                    else:
                        st.info("💡 실적은 등록되었으나, 보장분석 시트에는 이미 동일한 계약이 있어 중복 입력을 건너뛰었습니다.")
                    st.rerun()
            ...
            for _, row in df.iterrows():
                ...
                if not exist.empty:
                    ...
                    for k, v in in_d.items():
                        if exist.iloc[0].get(k, "") in ["", "-"] and v:
