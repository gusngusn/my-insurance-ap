...
with st.sidebar:
    st.header("📋 관리 메뉴")
    menu_options = ["📊 메인 대시보드", "🔍 고객 조회 및 수정", "➕ 고객 신규등록", "📄 보유계약 입력", "💰 실적 입력 및 분석", "📂 CSV DB 일괄 업로드", "💬 무료 PC카톡 자동 발송"]
...
elif st.session_state.menu == "💬 무료 PC카톡 자동 발송":
    st.title("💬 무료 PC카톡 매크로 자동 발송")
    st.warning("🚨 [주의사항] 매크로가 시작되면 **절대 마우스와 키보드를 만지지 마세요!** 화면이 혼자 움직입니다.")
    st.markdown("""
    **💡 PC 카톡 발송 준비 3단계**
    1. 지금 시스템이 켜져 있는 현우님의 **윈도우 PC(로컬 환경)**에서 실행해야 작동합니다.
    2. PC **카카오톡이 켜져 있고 로그인** 되어 있어야 합니다. (화면에 카톡 창을 띄워두세요)
    3. 고객 DB의 '이름'과 카카오톡 친구 목록에 저장된 '이름'이 똑같아야 합니다.
    """)
    
    valid_cust = db_cust[db_cust['연락처'].astype(str).str.len() > 8]
    
    st.subheader("1. 카톡을 보낼 대상 선택")
    target_group = st.radio("카톡 발송 대상을 선택하세요", ["직접 선택", "이번달 생일 고객"])
    ...
    if target_group == "이번달 생일 고객":
        ...
        selected_k_recipients = st.multiselect("수신 고객 선택", valid_cust['이름'].tolist())
