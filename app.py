...
st.set_page_config(page_title="현우 통합 관리 v4.9", layout="wide")
st.title("🛡️ 배현우 설계사 통합 관리 시스템 v4.9")
...
if sheet:
                    ...
                    with st.expander(f"👤 {row['이름']} (주민번호: {row['주민번호']})", expanded=True):
                        ...
                        if "[보장분석]" in memo_raw:
                            ...
                            for item in ins_items:
                                # 필터링: 미납, 해지, 실효 및 의미 없는 단어 제거
                                if any(k in item for k in ["미납", "해지", "실효", "종료", "보험사", "계약자", "납입주기"]):
                                    continue
                                    
                                parts = item.split('/')
                                if len(parts) >= 2:
                                    # 상품명 정제: 특수문자 제거 및 '무배당' 정리
                                    clean_name = re.sub(r'\(.*?\)|\[.*?\]', '', parts[1]).replace("무배당", "").replace("생명", "").strip()
                        ...
                        else: st.info("등록된 유지계약 데이터가 없습니다.")
    ...
    # [TAB 2] AI 정보 등록 (생략 없음)
    with tab2:
        st.subheader("📝 텍스트로 정보 등록 및 자동 병합")
        raw_text = st.text_area("이름, 연락처, 주민번호 등을 자유롭게 입력하세요", height=200)
        if st.button("🚀 데이터 분석 및 반영"):
            ...
            if name and phone:
                ...
                if match_idx:
                    ...
                    st.success(f"✅ {name} 고객님 업데이트 완료!"); st.rerun()
                else:
                    new_row = [datetime.now().strftime("%Y-%m-%d"), name, ssn, phone, addr, job, memo.strip(), name, 0, 0, 0, 0, "", "", ""]
                    sheet.append_row(new_row); st.success(f"✅ {name} 고객님 신규 등록 완료!"); st.rerun()
        ...
        if st.button("✅ 증권 정보 클라우드 반영"):
            ...
            if len(parts) >= 4:
                ...
                if idx_list:
                    ...
                    st.success(f"✅ {parts[0]}님 차량 정보 업데이트 완료!"); st.rerun()

    # [TAB 4] 보장분석 PDF 업로드 (생략 없음)
    with tab4:
        st.subheader("📄 보장분석 PDF 정밀 추출")
                ...
                try:
                    ...
                    for line in text.split('\n'):
                        ...
                        if company and ("보험" in line or "공제" in line):
                            ...
                            if len(name) > 2:
                                extracted_list.append(f"{company}/{name}/{price.group() if price else '-'}/{date.group() if date else '-'}")
                    ...
                    if unique_list:
                        ...
                        sheet.update_cell(row_num, 7, final_memo)
                        st.success(f"✅ {target_u}님 유효 계약 {len(unique_list)}건 반영 완료!"); st.rerun()
